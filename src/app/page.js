...
export default function Home() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('https://service-order-php.vercel.app/api/api/order-services');
        if (!response.ok) {
  ...
  const handleStatusChange = useCallback(async (id, newStatus) => {
    try {
      const response = await fetch(`https://service-order-php.vercel.app/api/api/order-services/update/${id}`, {
        method: 'PUT',
        headers: {
